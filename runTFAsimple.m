...
num_windows = floor((length(map) - window_length_n) / window_step_n) + 1;
...
[S_mapco2, ~] = cpsd(map_clean, co2_clean, window, window_overlap_n, fft_length_n, fs);
[S_co2map, ~] = cpsd(co2_clean, map_clean, window, window_overlap_n, fft_length_n, fs);
[S_co2co2, ~] = cpsd(co2_clean, co2_clean, window, window_overlap_n, fft_length_n, fs);

[S_mapcbv, ~] = cpsd(map_clean, cbv_clean, window, window_overlap_n, fft_length_n, fs);
[S_co2cbv, ~] = cpsd(co2_clean, cbv_clean, window, window_overlap_n, fft_length_n, fs);
...
%
%
% MY BOI TELLING ME TO THRESHOLD POWER
%
%

% Partial Coherence COME BACK TO UNDERSTAND THIS
