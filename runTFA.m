...
%% Define Spectral Lookup Functions

S_BP_BP    = @(k) BP_Sxx(k);
S_CO2_CO2  = @(k) CO2_Sxx(k);

S_BP_CO2   = @(k) BP_CO2_Sxy(k);
S_CO2_BP   = @(k) CO2_BP_Sxy(k);

S_BP_CBF   = @(k) BP_CBF_Sxy(k);
S_CO2_CBF  = @(k) CO2_CBF_Sxy(k);
...
for k = 1:length(f)
    ...
    S_matrix = [S_BP_BP(k),    S_BP_CO2(k);
                S_CO2_BP(k),   S_CO2_CO2(k)];
    ...
    if enoughBPpower && enoughCO2power && enoughCBFpower && matrixIsStable
        ...
        % More stable than inv(S_matrix) * S_vector
        H_vector = S_matrix \ S_vector;

        H_BP_CBF_MISO(k,1)  = H_vector(1,1);
        H_CO2_CBF_MISO(k,1) = H_vector(2,1);

        % Multiple coherence-like reliability measure:
        %
        % This estimates how much of the output power is explained
        % by both inputs together.
        %
        % numerator = explained output power
        % denominator = total output power
...
hold on;
plot(f_out, multipleCoherence_out, 'LineWidth', 1.5);
...
psdResults.BP_CO2_Sxy = BP_CO2_Sxy_out;
psdResults.CO2_BP_Sxy = CO2_BP_Sxy_out;

psdResults.H_BP_CBF_MISO = H_BP_CBF_MISO_out;
psdResults.H_CO2_CBF_MISO = H_CO2_CBF_MISO_out;

psdResults.gain_BP_CBF_MISO = gain_BP_CBF_MISO_out;
psdResults.gain_CO2_CBF_MISO = gain_CO2_CBF_MISO_out;

psdResults.phase_BP_CBF_MISO = phase_BP_CBF_MISO_out;
psdResults.phase_CO2_CBF_MISO = phase_CO2_CBF_MISO_out;

psdResults.BP_CBF_coherence = BP_CBF_coherence_out;
psdResults.CO2_CBF_coherence = CO2_CBF_coherence_out;
psdResults.BP_CO2_coherence = BP_CO2_coherence_out;

psdResults.multipleCoherence = multipleCoherence_out;
